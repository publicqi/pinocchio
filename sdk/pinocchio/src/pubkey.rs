...
pub fn try_find_program_address(seeds: &[&[u8]], program_id: &Pubkey) -> Option<(Pubkey, u8)> {
    #[cfg(target_os = "solana")]
    {
        // SAFETY: we only need to reserve (32 + 1) bytes to pass to the syscall.
        // Uninited dirty value won't be returned to the caller.
        let mut bytes = core::mem::MaybeUninit::<[u8; 32]>::uninit();
        let mut bump_seed = core::mem::MaybeUninit::<u8>::uninit();
        ...
        let result = unsafe {
            crate::syscalls::sol_try_find_program_address(
                ...
                bump_seed.as_mut_ptr() as *mut u8,
            )
        };
        match result {
            crate::SUCCESS => Some((unsafe { (bytes.assume_init(), bump_seed.assume_init()) })),
            _ => None,
        }
    }
...
pub fn create_program_address(
    ...
    #[cfg(target_os = "solana")]
    {
        // SAFETY: we only need to reserve 32 bytes to pass into the syscall.
        // Uninited dirty value won't be returned to the caller.
        let mut bytes = core::mem::MaybeUninit::<[u8; 32]>::uninit();
